# abmain_fixed_final_parenting_fix.py
"""
Final fixed version: Ensures ALL floating elements are children of the
main_frame container to correctly layer them above the background image (a.jpg).
Transparency and threading fixes are preserved.
"""
...
# ------------------ Root and Frame Setup ------------------

# Ensure ctk and Image are imported at the start of your full script
# from customtkinter import CTkImage
# from PIL import Image
# import customtkinter as ctk

# (Assuming other required variables like BASE_DIR exist)
ctk.set_appearance_mode("system")
root = ctk.CTk()
root.geometry("1080x1920")
root.title("Mr. Pillai — Andar Bahar")

# 1. Main container frame must be transparent and cover the root window
main_frame = ctk.CTkFrame(root, fg_color="transparent")
main_frame.place(relx=0.5, rely=0.5, anchor="center", relwidth=1, relheight=1)
# 2. Load and place the background image inside the main_frame
bg_path = os.path.join(BASE_DIR, "a.jpg")
bg_label = None # Initialize to None in case of failure

try:
    bg_img = ctk.CTkImage(Image.open(bg_path), size=(1080, 1920))
    # CRITICAL FIX: Parent the background image to main_frame
    bg_label = ctk.CTkLabel(main_frame, image=bg_img)
    bg_label.place(relx=0.5, rely=0.5, anchor="center",relwidth=1, relheight=1)
except Exception as e:
    print(f"⚠️ Failed to load background image 'a.jpg': {e}")
    # Handle the error gracefully if the image file is missing

# ------------------ WIDGET DEFINITIONS (ALL PARENTED TO main_frame) ------------------
...
# Add game label (Total Games)
game_label = ctk.CTkLabel(main_frame,  # <-- FIXED: Parent is main_frame
                          text="0",
                          font=ctk.CTkFont(size=70, weight="bold"),
                          text_color="#ffd36e",
                          fg_color="transparent") # <-- CRITICAL: Transparent background
game_label.place(relx=0.5, rely=0.30, anchor="center")

# Status Label (CASINO GOLD)
status_label = ctk.CTkLabel(main_frame,  # <-- FIXED: Parent is main_frame
                            text="CASINO GOLD",
...
andar_counter_label = ctk.CTkLabel(main_frame,  # <-- FIXED: Parent is main_frame
                                   text="0",
                                   font=ctk.CTkFont(size=80, weight="bold"),
                                   text_color="#8ef0c6",
                                   fg_color="transparent") # <-- CRITICAL: Transparent background
andar_counter_label.place(relx=0.25, rely=0.35, anchor="center")

bahar_counter_label = ctk.CTkLabel(main_frame, text="0",  # <-- FIXED: Parent is main_frame
                                   font=ctk.CTkFont(size=80, weight="bold"),
                                   text_color="#ff9c9c",
                                   fg_color="transparent") # <-- CRITICAL: Transparent background
bahar_counter_label.place(relx=0.75, rely=0.35, anchor="center")

# ------------------ FLOATING CARDS AND CONTROLS ------------------

# ANDAR Text
andar_label = ctk.CTkLabel(main_frame, text="ANDAR",  # <-- FIXED: Parent is main_frame
...
andar_img_label = ctk.CTkLabel(main_frame, text="", width=200, height=300, # <-- FIXED: Parent is main_frame
                               fg_color="transparent",
                               corner_radius=10)
andar_img_label.place(relx=0.25, rely=0.53, anchor="center")

# JOKER Text
joker_label = ctk.CTkLabel(main_frame, text="JOKER",  # <-- FIXED: Parent is main_frame
                           font=ctk.CTkFont(size=50, weight="bold"),
                           text_color="#ffd36e",
                           fg_color="transparent") # <-- CRITICAL: Transparent background
joker_label.place(relx=0.5, rely=0.40, anchor="center")

# Joker Card Image Label (Using fg_color="transparent" for the card box area too)
joker_img_label = ctk.CTkLabel(main_frame, text="NEXT ROUND", width=220, height=320, # <-- FIXED: Parent is main_frame
                               fg_color="transparent",
                               corner_radius=10)
joker_img_label.place(relx=0.5, rely=0.53, anchor="center")

joker_text = ctk.CTkLabel(main_frame, text="",  # <-- FIXED: Parent is main_frame
                          font=ctk.CTkFont(size=14), text_color="#8fffd6",
                          fg_color="transparent") # <-- CRITICAL: Transparent background
joker_text.place(relx=0.5, rely=0.52, anchor="center")

# BAHAR Text
bahar_label = ctk.CTkLabel(main_frame, text="BAHAR",  # <-- FIXED: Parent is main_frame
                           font=ctk.CTkFont(size=40, weight="bold"),
                           text_color="#ff9c9c",
                           fg_color="transparent") # <-- CRITICAL: Transparent background
...
bahar_img_label = ctk.CTkLabel(main_frame, text="", width=200, height=300, # <-- FIXED: Parent is main_frame
                               fg_color="transparent",
                               corner_radius=10)
bahar_img_label.place(relx=0.75, rely=0.53, anchor="center")

# Buttons
ctk.CTkButton(main_frame, text="New Round (/)", width=180,  # <-- FIXED: Parent is main_frame
              command=lambda: reset_game()).place(relx=0.36, rely=.9, anchor="center")
ctk.CTkButton(main_frame, text="Manual Andar (1)", width=160,  # <-- FIXED: Parent is main_frame
              command=lambda: manual_result("ANDAR")).place(relx=0.5, rely=.9, anchor="center")
ctk.CTkButton(main_frame, text="Manual Bahar (2)", width=160,  # <-- FIXED: Parent is main_frame
              command=lambda: manual_result("BAHAR")).place(relx=0.64, rely=.9, anchor="center")

# history
bead_canvas = ctk.CTkCanvas(main_frame, height=160, # <-- FIXED: Parent is main_frame
                            highlightthickness=0,)
bead_canvas.place(relx=0.5, rely=0.75, anchor="center", width=1200)

# ------------------ Final Layering and Run ------------------

# CRITICAL FIX: Ensure the background image is the lowest layer within main_frame
...
def process_token(raw_token):
    ...
    if not joker_card:
        ...
        # --- FIXED: Ensure all widget updates happen in the main thread ---
        if img:
            def update_joker(w, i):
                w.configure(image=i, text="")
                w.image = i  # <--- Critical: Set reference in main thread

            root.after(0, update_joker, joker_img_label, img)
        else:
            root.after(0, joker_img_label.configure, {"text": card_name, "image": None})
        ...
        if img:
            ...
            root.after(0, target_widget.configure, {"text": card_name, "image": None})
...
# CRITICAL LAYERING FIX: Ensure the background image is the lowest layer
if main_frame:
    main_frame.lift()
elif bg_label:
    bg_label.lower()
